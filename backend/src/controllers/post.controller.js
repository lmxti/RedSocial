...
async function getPosts(req, res) {
    try {
        ...
        if(postsError) return respondError(req, res, 400, "Error al buscar publicaciones");
        posts.length === 0
...
async function updatePost(req, res){
    try {
        ...
        if(bodyError) return respondError(req, res, 400, bodyError.message);
        const [updatedPost, updatedPostError] = await PostService.updatePost(id, body, userId);
        if(updatedPostError) return respondError(req, res, 400, updatedPostError);
        if(!updatedPost) return respondError(req, res, 400, "No se actualizo publicacion");
        respondSuccess(req, res, 201, { message: "Publicacion actualizada", data: updatedPost });
    } catch (error) {
...
async function deletePost(req, res){
    try {
        ...
        const [post, postError] = await PostService.deletePost(id, userId, userRole);
        if(postError) return respondError(req, res, 400, postError);
        if(!post) return respondError(req, res, 400, "No se elimino publicacion");
        respondSuccess(req, res, 200, { message: "Publicacion eliminada", data: post });
    } catch (error) {
